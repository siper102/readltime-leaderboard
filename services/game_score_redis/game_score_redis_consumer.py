...
class GameScoreRedisConsumer:
    ...
    def __init__(self):
        ...
        self.redis_obj = Redis(host=getenv("redis_host"), port=getenv("redis_port"), decode_responses=True)
    ...
    def game_score_redis_consumer(self, message):
        ...
        self.producer.send(
            topic=getenv("leaderboard_change_topic"),
            value=score_dto.get("created_at")
        )
