...
class LeaderboardCacheConsumer:
    ...
    def leaderboard_cache_consumer(self, message):
        ...
        if not self.can_update_leaderboard(leader_board_cache, current_time_ms):
            print(
                f"[LeaderboardCacheConsumer] Not updating leaderboard "
                f"for change {leaderboard_change}"
            )
            return
        leader_board = self.redis_obj.zrevrange(
            name=getenv("redis_game_score_db"), start=0, end=9, withscores=True
        )
        if not leader_board:
    ...
    def can_update_leaderboard(self, leader_board_cache, current_time_ms) -> bool:
        return (leader_board_cache is None) or (
            current_time_ms - leader_board_cache.get("current_time_ms")
            > self.duration_ms
        )
